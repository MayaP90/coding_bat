...
def describe_a_sleep_in_function():
    ...
    def that_determines_whether_or_not_we_should_sleep_in():
     '''
      The two things we care about if it's weekend/ weekday, 
     ...
     ''' 
     assert sleep_in(True, True) == True # weekday but on a vacation => sleep in
     assert sleep_in(True, False) = False # weekday but NOT vacation => do NOT sleep in
